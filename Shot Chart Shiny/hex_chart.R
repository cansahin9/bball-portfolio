...
library(scales)
percent_formatter <- label_percent(accuracy = 1)
...
generate_hex_chart = function(hex_data, base_court, court_theme = court_themes$dark,
                              metric = "bounded_fg_diff", alpha_range = c(0.85, 0.98)) {
  
  metric_sym <- rlang::sym(metric)  # Convert string to symbol here
  
  if (length(hex_data) == 0) {
    return(base_court)
  }
  
  if (as.character(metric) == "bounded_fg_diff") {
...
generate_shot_chart <- function(data, base_court, court_theme = court_themes$dark) {
  base_court +
    geom_point(
      data = shot_data,
      aes(x = loc_x, y = loc_y, color = isShotMade),
      alpha = 0.8,
      size = 2
    ) +
    scale_color_manual(
      values = c("TRUE" = court_theme$made, "FALSE" = court_theme$missed),
