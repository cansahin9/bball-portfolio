...
# ---- Load Data ----
team <- read_csv("ec_team_matchup_data.csv") %>% clean_names()
opp  <- read_csv("ec_opponent_matchup_data.csv") %>% clean_names()
...
logo_df <- map_dfr(unique(df$team_abb), function(code) {
  ...
  res <- euroleaguer::getTeam(team_code = code, season_code = "U2024")
  if (is.null(res)) {
