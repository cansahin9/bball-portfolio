...
gt_theme_f5 <- function(gt_object) {
  gt_object %>%
    ...
    tab_options(
      column_labels.background.color = "#49BED8", # team color
...
gt_percentile_table <- team_percentiles %>%
  ...
  tab_header(
    ...
    subtitle = "Percentile vs All Teams Across Rounds 1–5"
  ) %>%
  data_color(
...
# Define zone order and group structure

zone_levels <- c("Rim", "Paint (Non-RA)", "Midrange", "Corner 3", "Above Break 3")

zone_groups <- tribble(
  ~Zone,             ~Group,
  "Rim",             "Finishing",
  "Paint (Non-RA)",  "Finishing",
  "Midrange",        "Midrange",
  "Corner 3",        "3PT",
  "Above Break 3",   "3PT"
)
...
ggplot(zone_data) +
  ...
  labs(
    ...
    subtitle = "Compared to All Teams (Rounds 1–5)",
