...
generate_game_flow_chart <- function(season_code, game_code, team_code) {
  ...
  label_y <- ifelse(is.finite(max_y), max_y + 5, 15)
  print(net_quarters)
  ...
  ggplot(pbp, aes(x = game_seconds)) +
    ...
    geom_ribbon(aes(ymin = 0, ymax = diff_pos),
                fill = ifelse(team_code == summary$CodeTeamA, "#DC7A2F", "black"),
                alpha = 0.3) +
    ...
    geom_line(
      aes(y = diff),
      color = ifelse(team_code == summary$CodeTeamA, "#DC7A2F", "black"),
      linewidth = 1
    ) +
...
generate_game_flow_chart(season_code = "U2025", game_code = 36, team_code = "LLI")  # London Lions
generate_game_flow_chart(season_code = "U2025", game_code = 36, team_code = "NIN")  # Niners Chemnitz

ggsave("game_flow_lions_vs_niners1.png", width = 12, height = 6, dpi = 300)
