...
df <- read_csv("ec_coach_ranks_upd.csv") %>%
  mutate(
    ...
    across(matches("^(pct_fga|opp_pct_fga)_(atb|cr)_3pt$"), ~ scales::percent(., .1))
  ) %>%
  select(-wins, -losses, -w_pct_rank) %>%
  relocate(record, .before = net_rating)

# Filter for coach (16 metric columns only)
Alimpijevic <- df %>%
  filter(str_detect(coach_name, "Alimpijevic")) %>%
  arrange(desc(season)) %>%
  select(
    ...
    opp_efg_pct_rank, opp_fta_rate_rank, opp_tov_pct_rank, dreb_pct_rank, opp_pct_fga_atb_3pt_rank, opp_pct_fga_cr_3pt_rank,
    net_rating, off_rating, def_rating, pace,
    efg_pct, fta_rate, tm_tov_pct, oreb_pct, pct_fga_atb_3pt, pct_fga_cr_3pt,
    opp_efg_pct, opp_fta_rate, opp_tov_pct, dreb_pct, opp_pct_fga_atb_3pt, opp_pct_fga_cr_3pt
  )
...
Alimpijevic %>%
  gt() %>%
  tab_header(
    title = paste("Key Rankings of Dusan Alimpijevic Coached Teams (EuroCup)"),
    subtitle = "As of October 3, 2025"
  )%>%
  cols_label(
    team_abbreviation = "",
  ...
  gt_merge_stack(col1 = dreb_pct_rank, col2 = dreb_pct, small_cap = F, palette = c("black", "black"), font_weight = c("normal", "normal"), font_size = c("13px", "7px")) %>%
  ...
  gt_save_crop(file = "Alimpijevic.png", whitespace = 20, bg = "white")
