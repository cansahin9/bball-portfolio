...
game_data <- getGamePoints(season_code = "U2025", game_code = 10)

# Shot coordinate transformation — calibrated to EuroLeague court
shots <- game_data %>%
  filter(Player == "ZIZIC, ANTE") %>%
...
p1 <- plot_court(court_theme = court_themes$ppt)+
  ...
  ggtitle(label = "Ante ZIZIC vs Lietkabelis Panevėžys",
          subtitle = "EuroCup Round 1 (2025-26)") +
